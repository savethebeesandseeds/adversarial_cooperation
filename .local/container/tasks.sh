...
readonly ARTIFACT_ROOT="$WORKSPACE_ROOT/.local/evidence"
readonly TASK_SCRIPT="$WORKSPACE_ROOT/.local/container/tasks.sh"
...
die() {
  printf '.local/container/tasks.sh: %s\n' "$*" >&2
  exit 1
}
...
validate_parameters() {
  (($# == 1)) || die 'usage: .local/container/tasks.sh web|verify|serve'
  case "$1" in
...
prepare_run_repository() {
  ...
  [[ -f "$RUN_REPOSITORY/code/Makefile.config" &&
     -f "$RUN_REPOSITORY/doc/adversarial_cooperation.tex" &&
...
prepare_publication_stage() {
  assert_publication_destination_available
  [[ ! -L "$WORKSPACE_ROOT/.local" ]] || die 'the local artifact parent must not be a symbolic link'
...
publish_artifacts() {
  ...
  if [[ "$AC_MODE" == verify ]]; then
    ...
    cp "$RUN_REPOSITORY/.local/build/pdf/adversarial_cooperation.pdf" \
      "$PUBLICATION_STAGE/pdf/Adversarial-Cooperation.pdf"
    cp "$RUN_REPOSITORY/.local/build/pdf/adversarial_cooperation_short.pdf" \

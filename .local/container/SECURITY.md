...
recorded in [README.md](README.md) in this directory.
...
Project operations live in `/workspace/.local/container/tasks.sh`, separately from
dependency setup:
...
the write-once `.local/evidence/<run-id>/` path. This leaves pre-existing
...
8. `bash .local/container/tasks.sh verify` passes, and `serve` is reachable only over
   host loopback.
9. Every unrelated container, image, volume, and network retains its prior ID
   and state, and preserved `.local/evidence` evidence is unchanged.

...
Earlier entries and the first cleanup map retain their historical names.

Current organization migration (2026-09-12): the author explicitly authorized
cleaning and ordering this repository. The exact move list is
[`organization-moves.json`](organization-moves.json), with purpose, preservation
rules and verification in [`plans/repository-organization.md`](plans/repository-organization.md).
That migration authorizes RN-001's root spelling correction and consolidation
of historical generated output and backups. Earlier entries below remain
historical proposals unless named by the current migration.
...
|---|---|---|---|---|---|
| RN-001 | `demostrations/` | `demonstrations/` | Demonstrations | Shared make configuration, book helper, container snapshot, active instructions and reference-vector commands | **Complete, 2026-09-12.** Author-authorized organization; generated files, placeholders and old poker note preserved separately according to the exact move map. |
...
30 tracked `.gitkeep` files in 18 demonstration directory trees; these are now
preserved locally in `.local/archive/demonstration-placeholders/`. Canonical
chapter directories were not renamed by the organization task.

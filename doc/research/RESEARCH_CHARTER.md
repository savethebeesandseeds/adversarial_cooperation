...
edition map is maintained in doc/research/EDITION_MAP.md.
...
Maintain `doc/research/CLAIM_LEDGER.md` with columns similar to:
...
Create a primitive registry in `doc/research/PRIMITIVE_REGISTRY.md`.
...
- `code/include/ac/` for public interfaces;
- `code/src/primitives/` for reusable primitive wrappers;
- `code/src/protocols/` for protocol state machines;
- `demos/` for chapter demonstrations;
- `code/tests/` for unit, integration, attack, and transcript tests;
- `code/tests/vectors/` for deterministic vectors;
- `doc/` for the book;
- `doc/research/` for claims, assumptions, plans, and open problems.

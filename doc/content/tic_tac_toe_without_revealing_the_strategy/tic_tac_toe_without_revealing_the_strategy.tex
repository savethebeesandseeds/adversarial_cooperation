...
\begin{sloppypar}
The preserved public-checker interface is \path{code/include/ac/ttt.h}; both
evaluators remain in \path{code/src/protocols/ttt.c}; and their tests remain in
\path{code/tests/test_ttt.c}. The Boolean interface is split between
\path{code/include/ac/bool_circuit.h} and
\path{code/include/ac/ttt_core_circuit.h}; its implementations are
\path{code/src/circuits/bool_circuit.c} and
\path{code/src/circuits/ttt_core_circuit.c}; and its tests are
\path{code/tests/test_bool_circuit.c} and
\path{code/tests/test_ttt_core_circuit.c}. The demonstration is
\path{code/src/tic_tac_toe_without_revealing_the_strategy/ttt_demo.c}. The protected
...
\path{code/include/ac/ttt_bind.h}; its wrapper and symbolic measurement are in
\path{code/src/protocols/ttt_bind.c}; its focused tests are
\path{code/tests/test_ttt_bind.c}; and its warning-bearing driver is
\path{code/src/tic_tac_toe_without_revealing_the_strategy/ttt_bind_demo.c}.
This component reuses the repository's existing
\path{code/src/primitives/commitment.c} and therefore links libsodium for
...
in \path{code/tests/vectors/tic-tac-toe-reference-v1.txt} uses the valid X reference

...
\begin{description}
    ...
    \path{code/src/protocols/rps.c} owns the parser,
    state transitions, opening verification, outcome, and transcript.
    \item[Teaching driver.] \leavevmode\\
    \path{code/src/two_oracles_play_rock_paper_scissors/rps_demo.c} creates two
    ...
    \begin{itemize}
        \item \path{code/tests/test_commitment.c}\par
        \noindent Deterministic commitment and mutation checks.\par
        \item \path{code/tests/test_rps.c}\par
        \noindent Honest games and core state behavior.\par
        \item \path{code/tests/test_rps_adversarial.c}\par
        Malformed, replay, invalid-opening, abort, timeout, and transcript
        attacks.\par
        \item \path{code/tests/vectors/commitment-v1.md}\par

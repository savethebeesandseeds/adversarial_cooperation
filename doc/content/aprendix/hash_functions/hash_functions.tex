...
\path{code/tests/vectors/hash-foundations-v1.md}; the untouched backup preserves the
original draft.
...
The public interface is \path{code/include/ac/hash.h}; the implementation is
\path{code/src/primitives/hash.c}; and the teaching driver is
\path{code/src/hash_functions/hash_demo.c}.
...
\path{code/tests/vectors/hash-foundations-v1.md}. The C test in
\path{code/tests/test_hash.c} checks:
